...
// CheckAndCreateTx is like CheckAndCreate but runs within an existing transaction.
func (s *AlertService) CheckAndCreateTx(ctx context.Context, tx pgx.Tx, t *domain.Telemetry, limits *domain.TelemetryLimits) ([]*domain.Alert, error) {
	type check struct {
		param   string
		value   *float64
		warnMin *float64
	...
	for _, c := range checks {
		...
		has, err := s.repo.HasActiveAlertTx(ctx, tx, t.LocomotiveID, c.param, severity)
		if err != nil {
			return created, fmt.Errorf("alert.CheckAndCreateTx HasActiveAlertTx: %w", err)
		}
		if has {
			continue
		}
		...
		id, err := s.repo.InsertTx(ctx, tx, alert)
		if err != nil {
			return created, fmt.Errorf("alert.CheckAndCreateTx InsertTx: %w", err)
		}
		alert.ID = id
		created = append(created, alert)
	}
